...
WORKDIR /src
COPY ["FastDostavka/FastDostavka.csproj", "FastDostavka/"]
RUN dotnet restore "FastDostavka/FastDostavka.csproj"
COPY . .
WORKDIR "/src/FastDostavka"
RUN dotnet build "FastDostavka.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "FastDostavka.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "FastDostavka.dll"]